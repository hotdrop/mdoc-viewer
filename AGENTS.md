...
実装や開発タスクの完了後、完了報告にルールメンテナンス観点のフィードバックを含める。

- 既存ルールが障壁になった、またはうまく対応できなかった点を洗い出す。なければ「なし」と記載する。
- 改善した方がよいルール、または追加した方がよいルールを洗い出す。なければ「なし」と記載する。
- 洗い出した候補を、特定の実装やタスクに限定できるものと、恒久的に適用できるものに分類する。
- 特定の実装やタスクに限定できるものは `.codex/skills/md-doc-viewer/SKILL.md` の更新候補とする。
- 恒久的に適用できるものは `AGENTS.md`、または `AGENTS.md` から参照するルールファイルの更新候補とする。
- ルール更新は、ユーザーの明示的な合意がある場合のみ実施する。
